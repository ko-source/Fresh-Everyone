...
const Shop = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("shop").get();
      ...
      buttons: [
        {
          label: "Yes",
          onClick: async() => {
            await firebase.firestore().collection("shop").doc(rowId).delete();
            alert("Shop Deleted");
            history.push('/');
            history.replace("/delivery/shop");
            // getVideos();
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Shop List</CCardHeader>
          <CCardBody>
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "centerName", label:"Shop Name" },
