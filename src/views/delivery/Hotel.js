...
const Hotel = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("hotel").get();
    ...
    const resolvedVideos = videos.docs.map((video) => {
      ...
      return {
        ...
        centerName:videoData.hotelName,
      ...
      buttons: [
        {
          label: "Yes",
          onClick: async() => {
            await firebase.firestore().collection("hotel").doc(rowId).delete();
            alert("Hotel Deleted");
            history.push('/');
            history.replace("/delivery/hotel");
            // getVideos();
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Hotel List</CCardHeader>
          <CCardBody>
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "centerName", label:"Hotel Name" },
