...
const RiderReport = (props) => {
  ...
  const getVideos = async () => {
    ...
    let resolvedVideos = videos.docs.filter((sub)=>sub.data().riderId==props.location.state.id).map((video) => {
      const id = video.id;
      ...
      <CCard className="mb-3" style={{ maxWidth: '540px',marginLeft:"18px" }}>
            ...
            <CCol md={8}>
                <CCardBody>
                    ...
                    <CCardText>
                        Name: {props.location.state.name+" "+props.location.state.surName}
                    </CCardText>
                    {props.location.state.phoneNumber==""?<CCardText></CCardText>:<CCardText>
                        Phone No.: {props.location.state.phoneNumber}
                    </CCardText>}
                    <CCardText>
                        Total Order Amount: <b>₹ </b>{wallet}
                    </CCardText>
                    <CCardText>
                        Total Delivery Charges: <b>₹ </b>{rTotal} 
                    </CCardText>
                    {/* <CCardText>
