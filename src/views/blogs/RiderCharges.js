...
const RiderCharges = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("RiderChargesWithdraw")
    .orderBy("date","desc")
    .get();
  ...
  const hist = async(rowId , name ,amt) => {
   console.log(rowId , name)
    const data1 = await firebase.firestore().collection("RiderChargesTotal").where("riderName" , "==" , name)
    ...
    // console.log(resolvedVideos[0])
      await firebase.firestore().collection("RiderChargesTotal").doc(resolvedVideos[0].id).update({
        amount : resolvedVideos[0].amount-amt,
      });
      await firebase.firestore().collection("RiderChargesWithdraw").doc(rowId).update({
        amount : 0,
      });
      alert("SuccessFull")
      getVideos();
