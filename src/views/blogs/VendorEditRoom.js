...
const VendorEditRoom = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("VendorRoomEdit").get();
      ...
      buttons: [
        {
          ...
          onClick: async() => {
            ...
            await firebase.firestore().collection("VendorRoomEdit").doc(rowId).delete();
            setRefresh(!refresh);
                alert("Room Deleted");
          },
        },
  ...
  const toggle = async(rowId,colId,token) => {
    ...
    // console.log(mob1);
   if(colId===true){
    await firebase.firestore().collection("VendorRoomEdit").doc(rowId).update({
  ...
  const toggle3 = async(rowId,colId,videoData) => {
    ...
    if(colId===true){
     ...
     await firebase.firestore().collection("VendorRoomEdit").doc(rowId).delete();
     await firebase.firestore().collection("vendor_rooms").doc(rowId).set({
        advanceSecurity:videoData.advanceSecurity,
        cleaningFacility:videoData.cleaningFacility,
        descOfAccomodation:videoData.descOfAccomodation,
        distanceFromCoaching:videoData.distanceFromCoaching,
  ...
  return (
    <CRow>
      ...
      <CCol>
        ...
        <span className="font-xl">Vendor Room Management</span>
            {/* <span>
          ...
          <CCardBody>
            ...
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "edit", label: "Approve Edit Request", filter: true },
                  ...
                  edit: (item, index) => {
                    ...
                    return (
                      <td>
                            ...
                            checked={item.isEdit}
                            onChange={async (e) => {
                              toggle3(item.id,item.isEdit,item)
                            }}
                            />                         
                      </td>
                    );
