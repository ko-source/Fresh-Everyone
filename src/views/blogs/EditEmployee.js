...
const EditEmployee = (props) => {
  ...
  const initialFormData = {
    fName:props.location.state.employeeName,
    dob:props.location.state.aadharNumber,
    email:props.location.state.email,
    userName:props.location.state.code,
  ...
  const handleSubmit = async (e) => {
    ...
    if (image.length === 0) {
      try {
        await firebase.firestore().collection("EmpCode").doc(props.location.state.id).update({
            employeeName:formData.values.fName,
