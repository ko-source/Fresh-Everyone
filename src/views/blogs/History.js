...
//   const [lastOrder, setLastOrder] = useState("");
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("report").doc(props.location.state.id).collection("everydayActivity")
    // .where("time", ">=", order)
    // .where("time", "<=", porder)
    .get();
  ...
  return (
    <CRow>
        <CCard className="mb-3" style={{ maxWidth: '540px',marginLeft:"18px" }}>
            ...
            <CCol md={8}>
                <CCardBody>
                    ...
                    {props.location.state.age==""?<CCardText></CCardText>:<CCardText>
                        Age : {props.location.state.age}
                    </CCardText>}
                    {props.location.state.country==""?<CCardText></CCardText>:<CCardText>
                        Country : {props.location.state.country}
                    </CCardText>}
                    {/* <CCardText>
      ...
      <CCol lg={12}>
        ...
        <CRow>
                <CCol sm="4">
                    <div className="font-xl">User History</div>
                </CCol>
                <CCol sm="1"></CCol>
                <CCol sm="2">
          ...
          <CCardBody>
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "time", label: "Breathe Time", filter: true },
