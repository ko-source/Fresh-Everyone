...
const WithdrawRider = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("RiderWithdrawRequest")
    .orderBy("date","desc")
    .get();
  ...
  const hist = async(rowId , name ,amt) => {
   console.log(rowId , name)
    const data1 = await firebase.firestore().collection("RiderGiveAmountToVendor").where("riderName" , "==" , name)
    ...
    // console.log(resolvedVideos[0])
      await firebase.firestore().collection("RiderGiveAmountToVendor").doc(resolvedVideos[0].id).update({
        amount : resolvedVideos[0].amount-amt,
      });
      await firebase.firestore().collection("RiderWithdrawRequest").doc(rowId).update({
        amount : 0,
      });
      alert("SuccessFull")
      getVideos();
