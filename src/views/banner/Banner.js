...
const Banner = () => {
  ...
  const getVideos = async () => {
    setLoading(true);
    const videos = await firebase.firestore().collection("BannerAds").get();
  ...
  const toggle = async(rowId,colId) => {
   if(colId===true){
    await firebase.firestore().collection("BannerAds").doc(rowId).update({
      isActive:false,
    })
    getVideos();
   }else{
      ...
      buttons: [
        {
          label: "Yes",
          onClick: async() => {
            await firebase.firestore().collection("BannerAds").doc(rowId).delete();
          ...
          <CCardBody style={{textAlign:"center"}}>
            <CDataTable style={{border:"5px solid black",textAlign: "center"}}
              ...
              fields={[
                ...
                { key: "image", label:"Banner Image",filter: false }, 
                // { key: "isActive", label: "Is Active",filter: true },
                // { key: "status" },
                { key: "show_delete", label: "Action",filter: false },
              ]}
              scopedSlots={{
                srno: (item, index) => {
                  return (
