...
const EditInstagram = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (window.vname == 0) {
    try {
        await firebase.firestore().collection("generalData").doc("banners").collection("instagram").doc(props.location.state.id).update({
  ...
  return (
    <CCard>
      <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Edit Instagram Image</CCardHeader>
      <CCardBody>
