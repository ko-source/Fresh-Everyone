...
//   }
      ...
      buttons: [
        {
          ...
          onClick: async() => {
            await firebase.firestore().collection("generalData").doc("data").update({
                units : firebase.firestore.FieldValue.arrayRemove(rowId)
            })
            alert("Unit Deleted");
            history.push('/');
            history.replace("/videos/unit");
            // getVideos();
            // setRefresh(!refresh);
  ...
  return (
    <CRow>
      ...
      <CCol>
        <CCard>
        <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Unit List</CCardHeader>
          <CCardBody>
            <CDataTable style={{border:"1px solid #ebedf0"}}
              ...
              fields={[
                ...
                { key: "cat", label: "Units",filter: true },
                // { key: "status" },
