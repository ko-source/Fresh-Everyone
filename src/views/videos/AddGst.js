...
const AddSubcat = () => {
    ...
    try {
        socPrice.forEach(async(item)=>{
            await firebase.firestore().collection("generalData").doc("data").update({
                gst : firebase.firestore.FieldValue.arrayUnion(item.gst)
  ...
  return (
    <CCard>
      <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Add GST</CCardHeader>
      <CCardBody>
