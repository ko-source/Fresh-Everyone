...
const AddBrand = () => {
    ...
    try {
        socPrice.forEach(async(item)=>{
            await firebase.firestore().collection("generalData").doc("brand").update({
                brand : firebase.firestore.FieldValue.arrayUnion(item.brand)
  ...
  return (
    <CCard>
      <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Add Brand</CCardHeader>
      <CCardBody>
          ...
          <CFormGroup>
                {
                  socPrice.map((socPrice, index)=>(
                    <CRow className="g-3 align-items-center">
                        <CCol md="6" sm="4">
                            <CLabel>Enter Brand Name</CLabel>
                        </CCol>
                        <CCol sm={5}>
