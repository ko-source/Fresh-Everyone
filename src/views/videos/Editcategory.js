...
const Editcategory = (props) => {
  ...
  const initialFormData = {
    cname: props.location.state.ques,
    answer:props.location.state.ans,
    priority:""
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (window.vname == 0) {
      try {
        await firebase.firestore().collection("previousQuestions").doc(props.location.state.id).update({
  ...
  return (
    <CCard>
      <CCardHeader style={{ fontWeight: "bold",backgroundColor:"#f7f7f7",fontSize:"1.1rem",color: "black"}} >Update Questions</CCardHeader>
      <CCardBody>
